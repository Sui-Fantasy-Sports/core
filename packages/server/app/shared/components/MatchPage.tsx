...
const GameHeader = ({
  ...
  <div className="col-span-full bg-black border border-[#2c2c2c] rounded-lg p-2 xs:p-3 sm:p-4 text-white font-orbitron">
    <div className="flex flex-col sm:flex-row justify-between items-start gap-2 xs:gap-3">
      <div className="flex-1 space-y-1 xs:space-y-2 sm:space-y-3 pr-0 sm:pr-3">
        <div className="shine text-base xs:text-lg sm:text-xl md:text-2xl">
          {seriesName}
        </div>
        <div className="flex items-center space-x-1 xs:space-x-2 text-xs xs:text-sm sm:text-base md:text-lg font-medium">
          <div>{teamA}</div>
...
const YourSelection = ({ selectedPlayers }: { selectedPlayers: SelectedPlayer[] }) => {
  ...
  return (
    <div className="bg-black text-white p-2 xs:p-3 sm:p-4 md:p-5 rounded-xl border border-[#2C2C2C] font-orbitron flex flex-col h-[60vh] sm:max-h-[500px]">
      <h2 className="text-base xs:text-lg sm:text-xl font-medium mb-2 xs:mb-3 sm:mb-4 border-b border-gray-700 pb-2">
        YOUR SELECTION
      </h2>
        ...
        {selectedPlayers.length === 0 ? (
          ...
          <div className="space-y-2 xs:space-y-3 sm:space-y-4">
            {selectedPlayers.map((p) => (
              <div
                key={p.id}
                className="flex items-center bg-white/5 backdrop-blur-md rounded-xl p-1 xs:p-2 sm:p-3 border border-white/10"
              >
                <div className="w-10 h-10 xs:w-12 xs:h-12 sm:w-14 sm:h-14 rounded-lg mr-2 xs:mr-3 sm:mr-4 overflow-hidden">
                  <img
                    src={p.playerImg}
                    alt={p.name}
                ...
                <div className="flex-1 py-1">
                  <div className="font-normal text-white text-xs xs:text-sm sm:text-sm py-1 leading-tight">{p.name}</div>
                  <div className="text-[10px] xs:text-xs sm:text-xs text-gray-400 leading-tight">{p.teamName}</div>
                </div>
                <div className="flex flex-col items-end">
                  <div className="bg-[#8b0000] text-white text-[10px] xs:text-xs sm:text-sm px-1 xs:px-1.5 sm:px-2 py-0.5 rounded-full">
                    x{p.quantity}
                  </div>
                  <div className="text-yellow-400 text-[10px] xs:text-xs sm:text-sm font-semibold mt-1">{p.tokenAmount || 0} MIST</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-3 xs:mt-4 sm:mt-6 border-t border-gray-700 pt-2 xs:pt-3 sm:pt-4 text-xs xs:text-sm sm:text-base">
        <div className="text-[10px] xs:text-xs sm:text-sm text-gray-400">NFTs</div>
        <div className="relative h-2 bg-gray-700 rounded-full overflow-hidden">
          <div
            className="absolute top-0 left-0 h-2 bg-gradient-to-r from-red-400 to-yellow-300"
            style={{ width: `${(totalNFTs / 40) * 100}%` }}
          ></div>
        </div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-yellow-400">{totalValue} MIST</span>
        </div>
      </div>
    </div>
  );
...
const TierTabs = ({
  ...
  packageId: string;
  isMatchCompleted: boolean;
  ownedNfts: { [playerId: string]: any[] };
  playerIdToIndex: { [playerId: string]: number };
}) => {
  const [activeTeams, setActiveTeams] = useState<{ [tier: string]: string }>({});
  ...
  return (
    <Tabs
      defaultValue="Tier 1"
      orientation="vertical"
      className="flex flex-col sm:grid sm:grid-cols-4 col-span-full gap-1 xs:gap-2 sm:gap-3 p-1 xs:p-2 sm:p-3 z-20 h-[60vh] sm:max-h-[500px] border border-neutral-800 rounded-lg"
    >
      <div className="col-span-1 self-start bg-black border border-neutral-800 rounded-lg px-1 xs:px-2 sm:px-3 py-1 xs:py-2 sm:py-3 text-white overflow-y-auto">
        <div
          className="flex items-center gap-1 text-base xs:text-lg sm:text-xl mb-1 xs:mb-2 sm:mb-3 border-b border-gray-700 pb-1 text-white tracking-wide"
        >
          <span>SELECT TIER</span>
        </div>
        <TabsList className="flex flex-col items-start gap-2 xs:gap-3 sm:gap-4 pt-1 xs:pt-2 sm:pt-3 bg-black">
          {["Tier 1", "Tier 2", "Tier 3"].map((tier, index) => (
            ...
            >
              <div className="flex flex-col items-start space-y-1">
                <span className="text-white">{tier}</span>
                <span className="text-[10px] xs:text-xs sm:text-sm text-yellow-400">
                  Bet: {index === 0 ? "300 MIST" : index === 1 ? "200 MIST" : "100 MIST"}
                </span>
              </div>
            </TabsTrigger>
          ))}
      ...
      <div className="col-span-3 backdrop-blur-sm rounded-lg bg-white/10 flex flex-col h-full overflow-y-scroll custom-horizontal-scrollbar">
        {["Tier 1", "Tier 2", "Tier 3"].map((tier) => {
          const allPlayers = [
            ...(playerData[tier]?.["TEAM A"] || []),
            ...(playerData[tier]?.["TEAM B"] || []),
          ];
          const uniqueTeams = [...new Set(allPlayers.map((p) => p.teamName).filter((team) => team !== undefined))] as string[];
          const activeTeam = activeTeams[tier] || (uniqueTeams.length > 0 ? uniqueTeams[0] : "No Team");
          const playersToShow = allPlayers.filter((p) => p.teamName === activeTeam);

          return (
            <TabsContent key={tier} value={tier} className="m-0 p-0 flex-1 flex flex-col">
              <div className="w-full relative flex flex-col h-full">
                    ...
                    uniqueTeams.map((team, index) => (
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center space-x-1 xs:space-x-2 px-1 xs:px-2 sm:px-3 py-1 xs:py-1.5 sm:py-2 transition-all duration-200
                          ...
                          ${index === 0 ? "rounded-tl-lg" : index === uniqueTeams.length - 1 ? "rounded-tr-lg" : ""}`}
                      >
                        <span className="text-xs xs:text-sm sm:text-base font-medium text-white tracking-wider">
                          {team}
                        </span>
                      </button>
                    ))
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-2 xs:gap-4 sm:gap-6 mt-1 xs:mt-2 sm:mt-3 px-1 xs:px-2 sm:px-3 py-1">
                    {playersToShow.length > 0 ? (
                      playersToShow.map((p) => (
                        <div key={p.id} className="w-full max-w-full">
                          <NftCard
                            player={p}
                            teamName={p.teamName || "Unknown Team"}
                            contestId={contestId}
                            playerId={p.id}
                            playerIndex={playerIdToIndex[p.id] || 0}
                            playerName={p.name}
                            packageId={packageId}
                            isMatchCompleted={isMatchCompleted}
                            tier={parseInt(tier.split(" ")[1])}
                            ownedNfts={ownedNfts[p.id] || []}
                            onQuantityChange={(playerId, qty) => {
                              onSelectPlayer(p, qty);
                              console.log(`Player ${p.name} now has ${qty} NFTs with id ${p.id}`);
                            }}
                          />
                        </div>
                      ))
                    ) : (
                      <div className="col-span-full flex items-start justify-center text-center text-gray-500 font-semibold text-xs xs:text-sm sm:text-base">
                        No players available in {activeTeam} for {tier}.
                      </div>
                    )}
                  </div>
...
export default function MatchPage() {
  const { contestId } = useParams<{ contestId: string | undefined }>();
  const suiClient = useSuiClient();
  const currentAccount = useCurrentAccount();
  const [matchData, setMatchData] = useState<Team[]>([]);
  const [isMatchCompleted, setIsMatchCompleted] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [ownedNfts, setOwnedNfts] = useState<{ [playerId: string]: any[] }>({});
  const [seriesName, setSeriesName] = useState<string>('Tournament');
  const [playerIdToIndex, setPlayerIdToIndex] = useState<{ [playerId: string]: number }>({});
  const packageId = "0x85cd910e9f4dd3720bdbf654d371ad5e2a8b5fbe52064f1c21f4a03682ad1846";
  ...
  useEffect(() => {
    const fetchMatchData = async () => {
      ...
      try {
        ...
        // Build playerIdToIndex mapping
        const playerIdToIndex: { [playerId: string]: number } = {};
        let index = 0;
        matchData.forEach((team) => {
          team.players.forEach((player) => {
            playerIdToIndex[player.id] = index++;
          });
        });

        if (matchData.some((team) => team.players.some((player) => player.tier === undefined))) {
          console.log("Some players are missing tiers, fetching from /api/players/tiers");
          const playerIds = matchData.flatMap((team) => team.players.map((p) => p.id));
          const playerTiers = await fetchPlayerTiers(playerIds);
          matchData = matchData.map((team) => ({
        ...
        setMatchData(matchData);
        setIsMatchCompleted(contest?.matchEnded || false);
        setPlayerIdToIndex(playerIdToIndex);

        let fetchedSeriesName = contest?.seriesName;
        if (!fetchedSeriesName && contest?.seriesId) {
          fetchedSeriesName =
            contest.seriesId === 'd5a498c8-7596-4b93-8ab0-e0efc3345312'
              ? "IPL '25"
              : SUPPORTED_SERIES.find((series) => series.id === contest.seriesId)?.name || 'Tournament';
        }
        setSeriesName(fetchedSeriesName || 'Tournament');
    ...
    fetchMatchData();
  }, [contestId, currentAccount, suiClient]);

  const fetchOwnedNftsForAllPlayers = async (teams: Team[], contestId: string) => {
    if (!currentAccount) {
      console.log("No current account, skipping fetchOwnedNftsForAllPlayers");
      return { nfts: {}, playerIdToIndex: {} };
    }
    ...
    try {
      ...
      // Build playerIdToIndex from matchData
      const playerIdToIndex: { [playerId: string]: number } = {};
      let index = 0;
      teams.forEach((team) => {
        team.players.forEach((player) => {
      ...
      for (const entry of playerNftIds.data) {
        ...
        const nftId = (entry.name as any).value;
        const playerIdx = Number((field.data?.content as any)?.fields?.value ?? -1);
        if (nftId && playerIdx !== -1) {
          nftIdToPlayerIndex[nftId] = playerIdx;
        }
      }

      // Map NFTs to player.id
      const nftsByPlayerId: { [playerId: string]: any[] } = {};
      objects.data.forEach((obj) => {
        const nftId = obj.data?.objectId;
        if (!nftId) return;
        ...
        // Find the player.id corresponding to playerIdx
        const playerId = Object.keys(playerIdToIndex).find(
          (id) => playerIdToIndex[id] === playerIdx
        );
        if (!playerId) return;

        if (!nftsByPlayerId[playerId]) {
          nftsByPlayerId[playerId] = [];
        }
        nftsByPlayerId[playerId].push({
          id: nftId,
          redeemValue: Number((obj.data?.content as any)?.fields?.redeem_value ?? 0),
        });
      });

      return { nfts: nftsByPlayerId, playerIdToIndex };
    } catch (error) {
      console.error("Failed to fetch owned NFTs for all players:", error);
      return { nfts: {}, playerIdToIndex: {} };
    }
  ...
  const playerData = transformMatchData(matchData);
  const teamA = matchData[0]?.teamName || "TBD";
  const teamB = matchData[1]?.teamName || "TBD";
  const timeLeft = matchData[0]?.startTime
    ? matchData[0].startTime > Math.floor(Date.now() / 1000)
      ? `${Math.floor((matchData[0].startTime - Math.floor(Date.now() / 1000)) / 3600)}h ${Math.floor(((matchData[0].startTime - Math.floor(Date.now() / 1000)) % 3600) / 60)}min`
      : "In Progress"
    : "TBD";
  const tokensLeft = 30;
  ...
  return (
    <SuiClientProvider networks={networks} defaultNetwork="testnet">
      <WalletProvider>
        <div className="flex justify-center items-center bg-black text-white min-h-screen pt-[64px] p-1 xs:p-2 sm:p-3 font-orbitron">
          <Navbar />
          <div className="w-[95%] xs:w-[90%] sm:w-[90%] md:w-6xl lg:w-7xl xl:w-8xl 2xl:w-[1920px] p-1 xs:p-2 sm:p-3 md:p-4">
            {error ? (
              <div className="text-center py-4 xs:py-6 sm:py-8 text-white text-xs xs:text-sm sm:text-base">Failed to load player data...</div>
            ) : matchData.length > 0 ? (
              <>
                <div className="gap-1 xs:gap-2 sm:gap-3 pt-[32px] text-white">
                  <GameHeader
              ...
              <div className="text-center py-4 xs:py-6 sm:py-8 text-xs xs:text-sm sm:text-base">Loading player data...</div>
            )}
          </div>
        </div>
      </WalletProvider>
